...
def _setup(w: int, h: int):
    bd = core.Backdrop(w, h)
    _STATE.update(w=w, h=h, bd=bd)


def frame_at(i: int, fps: int = core.FPS) -> np.ndarray:
    w, h, bd = _STATE["w"], _STATE["h"], _STATE["bd"]
    t = i / fps
    fr = core.Frame(w, h)
    L = scenes.Layout(fr)
    scenes.render(fr, L, t)
    buf, glow = fr.compose()
    glow += bd.mist(t, scenes.mist_gain(t))
    return bd.finish(buf, glow, i, scenes.exposure(t))
...
def render_video(fmt: str, out: Path, fps: int, seconds: float, workers: int,
    ...
    with Pool(workers, initializer=_setup, initargs=(w, h)) as pool:
...
def main():
    ...
    ap.add_argument("--seconds", type=float, default=scenes.TEND)
    ...
    if a.stills:
        render_stills(a.format, [float(x) for x in a.stills.split(",")],
                      Path(a.stills_dir), a.fps, a.scale)
        return
    out = Path(a.out or f"out/ddr_{a.format}.mp4")
    render_video(a.format, out, a.fps, a.seconds, a.workers, a.scale, a.crf)
